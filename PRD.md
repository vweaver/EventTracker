...
**Platform.** Single-page web app, mobile-first. Local SQLite storage. All timestamps use device system time.

**Data Model.** Single SQLite table: `events(id integer primary key, timestamp datetime, value boolean)`.
...
**Out of Scope.** Timezone handling, rolling windows, multiple event types, user-defined blocks, forecasting, cross-user aggregation, server/sync.

**System in one sentence.** CRUD `{id, timestamp, boolean}` in local SQLite; on read, map all events into 28 day×block buckets and compute ratios.
